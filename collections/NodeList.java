...
public class NodeList{
	...
	public void p2pMessaging(String destination) {
		for(Iterator<User> iterator = ledger.iterator(); iterator.hasNext();) {
			User user = iterator.next();
	...
	public void consensus(String destination, Object data) {
		for(Iterator<User> iterator = ledger.iterator(); iterator.hasNext();) {
			User user = iterator.next();
			if(user.getMacAddress().matches(destination)) {
				addToChain(convertToBlock((String) data));
